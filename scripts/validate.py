...
def validate(hwpx_path: str) -> list[str]:
    ...
    if not path.is_file():
        return [f"File not found: {hwpx_path}. Check the file path and try again."]

    if not path.suffix.lower() == ".hwpx":
        errors.append(
            f"File '{path.name}' does not have .hwpx extension. "
            f"This tool supports .hwpx only, not .hwp (binary)."
        )
    ...
    try:
        ...
        return [
            ...
            f"The file may be corrupted or in .hwp (binary) format."
        ]
        ...
        for required in REQUIRED_FILES:
            if required not in names:
                errors.append(
                    ...
                    f"A valid HWPX must contain this file."
                )
