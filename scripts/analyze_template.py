...
def main():
    ...
    try:
        ...
        if not os.path.exists(header_path) or not os.path.exists(section_path):
            print(
                "ERROR: Contents/header.xml or Contents/section0.xml not found in archive.",
                file=sys.stderr,
            )
            print(
                "  The file may not be a valid HWPX document. "
                "Use validate.py to check its structure.",
                file=sys.stderr,
            )
            sys.exit(1)
